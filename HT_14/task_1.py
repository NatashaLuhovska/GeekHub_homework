...
class ATM:
	...
	IN_BANK_RATE_URL = urljoin(BASE_URL, 'p24api/pubinfo?json&exchange&coursid=5')
	# Безготівковий курс ПриватБанку (конвертація за картками, Приват24, поповнення вкладів):
	ONLINE_RATE_URL = urljoin(BASE_URL, 'p24api/pubinfo?exchange&coursid=11') 
	...
	def get_exchange_rate(self):
			...
			print('\n----------------------------------------------------- \n')
			print(' Безготівковий курс ПриватБанку \n(конвертація за картками, Приват24, поповнення вкладів): ')
			print(f" Валюта     Купівля       Продаж ")
			for currency in cours_list_online:
				print(f"   {currency['ccy']}      {currency['buy']}      {currency['sale']} ")
